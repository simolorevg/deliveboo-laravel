            ...
            <form method="POST" class="d-flex flex-column form "
                ...
                <div class="col-md-6 div-input">
                    ...
                    <input class="form-control @error('restaurant_name') is-invalid @enderror" type="text"
                        ...
                        value="{{ old('restaurant_name', $restaurant->restaurant_name) }}" required minlength="3"
                        maxlength="20">
                ...
                <input class="form-control @error('address') is-invalid @enderror" type="text" name="address"
                    id="address" value="{{ old('address', $restaurant->address) }}" required minlength="3"
                    maxlength="20">
                @error('address')
                    <div class="invalid-feedback">
                        {{ $message }}
                ...
                <input class="form-control @error('phone') is-invalid @enderror" type="number" name="phone"
                    id="phone" value="{{ old('phone', $restaurant->phone) }}" required minlength="9" maxlength="11">
                @error('phone')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <div class="btn-group editR mx-auto d-flex flex-wrap gap-2" role="group"
                    ...
                    @foreach ($categories as $category)
                        <input type="checkbox"
                            class="btn-check edit form-control @error('category_id') is-invalid @enderror"
                            id="{{ $category->category_name }}" autocomplete="off" name="category_id[]"
                            value="{{ $category->id }}"
                            {{ in_array($category->id, old('category_id', $restaurant->categories->pluck('id')->toArray())) ? 'checked' : '' }}>
