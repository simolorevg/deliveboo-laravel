        ...
        @else
            <div class="my-4 d-flex justify-content-end">
                <a class="btn btn-primary mx-1"
                    href="{{ route('admin.order.stats', ['restaurant_id' => Auth::user()->restaurant->id]) }}">Vai alle
                    statistiche degli ordini</a>

            </div>


            <table class="table stats table-dark table-hover table-striped">
                <thead>
                ...
                <tbody>
                    ...
                    @foreach ($paginatedOrders as $item)
                        <tr class="text-center">
                            ...
                            <td scope="row">{{ $item->created_at->format('H:i d-m-y') }}</td>
                            <td scope="row">{{ $item->guest_address }}</td>
                            <td scope="row">{{ $item->guest_phone }}</td>
                            <td scope="row">{{ $item->guest_mail }}</td>
                            <td scope="row" class="align-middle">
                                <a class="btn btn-primary mx-1" href="{{ route('admin.orders.show', $item) }}">&#128065;</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
